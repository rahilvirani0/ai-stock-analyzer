...
# Set basic page configuration (no extra descriptions)
st.set_page_config(page_title="Canadian Stock Forecaster", page_icon="📈", layout="wide")

# Minimal title
st.title("Canadian Stock Forecaster")
...
class StockForecaster:
    ...
    def prepare_data(self, stock_data):
        ...
        # Compute daily volatility (used for noise during forecasting)
    ...
    def load_or_retrain_model(self, ticker):
        """
        Checks if an up-to-date fine-tuned model exists in the "stock-models" folder.
        If not, retrains the model (which also deletes older models).
        """
    ...
    def generate_forecast(self, stock_data, scaled_data, forecast_days=180):
        ...
        # Get last sequence from scaled data for forecasting
        ...
        with st.spinner("Generating forecast..."):
            for _ in range(forecast_days):
                ...
                # Optionally add realistic noise
                ...
                # Constrain change to reasonable range
    ...
    def calculate_forecast_metrics(self, ticker, stock_data, base_forecast):
        # Use the last forecasted date based on forecast horizon
        ...
        if potential_return > 10:
            risk_assessment = "Bullish: Strong potential gains"
            risk_color = "green"
        elif potential_return > 0:
            risk_assessment = "Neutral: Modest gains expected"
            risk_color = "blue"
        else:
            risk_assessment = "Bearish: Potential losses ahead"
            risk_color = "red"

        # Placeholder model accuracy (could be updated with more detailed evaluation)
        model_accuracy = "High"
...
selected_ticker = st.sidebar.selectbox("Ticker", options=canadian_tickers, index=0)
forecast_horizon = st.sidebar.slider("Forecast Horizon (Days)", min_value=30, max_value=365, value=180, step=30)

if st.sidebar.button("Generate Forecast"):
    forecaster = StockForecaster()
    if not os.path.exists(forecaster.base_model_path):
        st.error("Base model file 'canadian_market_base_model.h5' not found.")
        st.stop()
    ...
    # Load or retrain model (only for Canadian stocks)
    ...
    # For the graph, show last forecast_horizon days of history plus forecast
    historical_dates = stock_data.index
    # Use the last 'forecast_horizon' days from historical data
    hist_range = historical_dates[-forecast_horizon:]
    historical_prices = stock_data['Close'].loc[hist_range].values.flatten()
    forecast_dates = pd.date_range(start=historical_dates[-1] + pd.Timedelta(days=1), periods=forecast_horizon)
    
    df_hist = pd.DataFrame({'Historical': historical_prices}, index=hist_range)
    df_forecast = pd.DataFrame({'Forecast': base_forecast}, index=forecast_dates)
    ...
    # Display forecast summary with color-coded texts
    st.markdown("### Forecast Summary")
    summary_html = f"""
    <p><b>Ticker:</b> {metrics['ticker']}</p>
    <p><b>Forecast Date:</b> {metrics['forecast_date']}</p>
    <p><b>Current Price:</b> ${metrics['current_price']:.2f}</p>
    <p><b>Forecast Price:</b> ${metrics['forecast_price']:.2f}</p>
    <p><b>Risk Assessment:</b> <span style="color:{metrics['risk_color']};">{metrics['risk_assessment']}</span></p>
    <p><b>Model Accuracy:</b> <span style="color:{metrics['accuracy_color']};">{metrics['model_accuracy']}</span></p>
    """
    st.markdown(summary_html, unsafe_allow_html=True)
else:
    st.info("Select a ticker and click 'Generate Forecast'.")